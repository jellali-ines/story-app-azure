...
const RecordingPanel = ({ currentSegment, story }) => (
  <div className="lg:col-span-1">
    <div className="bg-gray-800/80 backdrop-blur-sm border border-gray-700 rounded-3xl p-6 shadow-2xl max-h-[calc(100vh-120px)] overflow-y-auto custom-scrollbar">
...
function LunchStory({ darkMode }) {
  ...
  const {
    ...
    setShowAddToFolder,
    showCreateFolder
    // setShowCreateFolder removed - not used in this component
  } = useApp();
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <AnimatePresence>
            {evaluationMode && (
              <motion.div
                ...
                exit={{ opacity: 0, x: 20 }}
              >
                <RecordingPanel currentSegment={currentSegment} story={story} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="flex justify-center mt-10 mb-6">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
          >
            {evaluationMode ? "❌ Close Training Mode" : "🎤 Practice Reading"}
          </motion.button>
        </div>
      </div>
