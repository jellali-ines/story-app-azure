...
export default function SpeechEvaluation({ referenceText }) {
  ...
  const sendToBackend = async (audioBlob) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/evaluate-speech', {
        method: 'POST',
        body: formData
      });
      ...
      if (err.message.includes('Failed to fetch') || err.message.includes('NetworkError')) {
        setError('❌ Cannot connect to backend server. Please make sure Flask is running on http://localhost:5000');
      } else {
